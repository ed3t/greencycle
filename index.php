...
while ($info = mysqli_fetch_array($query_result))
{
    ...
    $totalAdsMaincat = get_items_count($config,false,"active",false,null,$info['cat_id']);
...
if($config['home_page'] == "home-map"){
    $page = new HtmlTemplate ('templates/'.$config['tpl_name'].'/home-map.html');
}
else{
...
$page->SetParameter ('OVERALL_HEADER', create_header($config,$lang,$lang['HOME'],$link));
...
$page->SetParameter('LATITUDE', get_option($config,"home_map_latitude"));
$page->SetParameter('LONGITUDE', get_option($config,"home_map_longitude"));
$page->SetParameter('MAP_COLOR', get_option($config,"map_color"));
$page->SetParameter('ZOOM', get_option($config,"home_map_zoom"));
