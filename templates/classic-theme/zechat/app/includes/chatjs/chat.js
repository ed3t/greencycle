...
var originalTitle;
var blinkOrder = 0;
var audioogg = new Audio(siteurl+'templates/'+template_name+'/zechat/app/includes/audio/chat.ogg');
var audiomp3 = new Audio(siteurl+'templates/'+template_name+'/zechat/app/includes/audio/chat.mp3');
...
emojione.imagePathPNG = siteurl+'templates/'+template_name+'/zechat/app/plugins/smiley/assets/png/';
emojione.imagePathSVG = siteurl+'templates/'+template_name+'/zechat/app/plugins/smiley/assets/svg/';
...
function chatWith(chatuser,chatheaderimg,status) {
    if(chatuser == session_uname){
        alert('Could not able to chat yourself.')
        return;
    }
...
function createChatBox(chatboxtitle,chatheaderimg,status,minimizeChatBox) {
    ...
    $("<div />" ).attr("id","chatbox_"+chatboxtitle)
        ...
        .html('<div class="chatbox-icon" onclick="javascript:toggleChatBoxGrowth(\''+chatboxtitle+'\')" href="javascript:void(0)"><div class="contact-floating red"><img class="chat-image img-circle pull-left" src="'+siteurl+'storage/profile/'+chatheaderimg+'"><small class="unread-msg">2</small><small class="status"></small></div></div>' +
    ...
    var scrollcode = $("#resultchat_"+chatboxtitle).scroll(function(){
        if ($("#resultchat_"+chatboxtitle).scrollTop() == 0){
            ...
            if($("#chatbox_"+client+" .pagenum:first").val() != $("#chatbox_"+client+" .total-page").val()) {
                ...
                var URL = siteurl+"plugins/zechat/chat.php?page="+pagenum+"&action=get_all_msg&client="+client;
    ...
    get_all_msg(siteurl+"plugins/zechat/chat.php?page=1&action=get_all_msg&client="+chatboxtitle);
...
function chatHeartbeat(){
    ...
    $.ajax({
        url: siteurl+"plugins/zechat/chat.php?action=chatheartbeat",
        cache: false,
        dataType: "json",
...
function checkChatBoxInputKey(event,chatboxtextarea,chatboxtitle) {
    ...
    if(event.keyCode == 13 && event.shiftKey == 0)  {
        ...
        if (message != '') {
            $.post(siteurl+"plugins/zechat/chat.php?action=sendchat", {to: chatboxtitle, message: message} , function(data){
...
function lastseen(uname){
    $.ajax({
        url: siteurl + "plugins/zechat/chat.php?action=lastseen&uname="+uname,
        cache: false,
        type: "POST",
        success: function (data) {
            if(data == "Online"){
...
function startChatSession(){
    $.ajax({
        url: siteurl+"plugins/zechat/chat.php?action=startchatsession",
        cache: false,
        dataType: "json",
        success: function(data) {
...
function closeChatBox(chatboxtitle) {
    ...
    $.post(siteurl+"plugins/zechat/chat.php?action=closechat", { chatbox: chatboxtitle} , function(data){
    });
...
function refreshTypingStatus(chatboxtitle){
    if (!$('#textarea').is(':focus') || $('#textarea').val() == '' || new Date().getTime() - lastTypedTime.getTime() > typingDelayMillis) {
        ...
        $.post(siteurl+"plugins/zechat/chat.php?action=typingstatus", {to: chatboxtitle, typing: 1} , function(data){
...
$(document).on('click', ".uploadFile", function (e){
    ...
    $(function() {
        ...
        $('#uploader_'+touname).plupload({
            ...
            url: siteurl+"upload-chat-file.php?tun="+touname,
